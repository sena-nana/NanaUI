...
- Public GPU extension contracts use \`nana-gpu\` types. \`wgpu::*\` may appear in a public signature of nana-gpu, nana-frame-exchange or nana-ui only behind \`wgpu-interop\`; \`nana_gpu::__framework\` is for the framework crates' own sources. \`scripts/check-engine-boundary.py\` enforces both.
