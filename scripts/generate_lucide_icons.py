#!/usr/bin/env python3
"""Generate nana-ui-core icon statics from Lucide SVG.
...
LUCIDE = "https://unpkg.com/lucide-static@0.544.0/icons/{name}.svg"
...
def emit_icon(const: str, lucide: str, svg: str) -> str:
    ident = snake(const)
...
def main() -> None:
    ...
    for const, lucide in SHELL + CATALOG:
        if lucide not in seen_svg:
            print(f"fetch {lucide}")
            seen_svg[lucide] = fetch(lucide)
        ident = snake(const)
        if const == "Sun":
            impl_lines.append("    pub const Sun: Self = Self(&APPEARANCE);")
            continue
        chunks.append(emit_icon(const, lucide, seen_svg[lucide]))
        chunks.append("")
