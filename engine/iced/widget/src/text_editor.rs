...
/// Returns whether the retained editor represented by `tree` is focused.
///
/// Compatibility adapters that recreate a [`TextEditor`] around externally
/// owned [`Content`] can use this without depending on the private widget state.
pub fn is_focused<Highlighter>(tree: &widget::Tree) -> bool
where
    Highlighter: text::Highlighter,
{
    tree.state
