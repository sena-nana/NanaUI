...
pub struct Font {
    ...
    /// User-space fvar coordinates applied to this instance (`(tag, value)`).
    pub(crate) variation_coords: Vec<(u32, f32)>,
}
...
impl Font {
    ...
    pub(crate) fn variation_coords(&self) -> &[(u32, f32)] {
...
impl Font {
    pub fn new(db: &fontdb::Database, id: fontdb::ID, weight: fontdb::Weight) -> Option<Self> {
        Self::new_with_variations(db, id, weight, &[])
    }
    ...
    pub fn new_with_variations(
        ...
        let mut location_settings = Vec::new();
        location_settings.push((Tag::new(b"wght"), weight.0 as f32));
        for &(tag, value) in variations {
            let axis_tag = Tag::from_u32(tag);
            if axis_tag == Tag::new(b"wght") {
                location_settings[0].1 = value;
                continue;
            }
            if axes.get_by_tag(axis_tag).is_some() {
                location_settings.push((axis_tag, value));
            }
        }
        let location = axes.location(location_settings.iter().copied());
        let variation_coords = location_settings
            .iter()
