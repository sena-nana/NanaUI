...
pub struct FontSystem {
    ...
    font_cache: HashMap<(fontdb::ID, fontdb::Weight, u64), Option<Arc<Font>>>,
...
impl FontSystem {
    ...
    pub fn get_font(&mut self, id: fontdb::ID, weight: fontdb::Weight) -> Option<Arc<Font>> {
        self.get_font_with_variations(id, weight, &crate::FontVariations::new())
    }

    /// Get a font by ID, weight, and extra fvar coordinates.
    ///
    /// Axes the face does not declare are ignored when the instance is built.
    pub fn get_font_with_variations(
        &mut self,
        id: fontdb::ID,
        weight: fontdb::Weight,
        variations: &crate::FontVariations,
    ) -> Option<Arc<Font>> {
        let hash = variations.hash64();
        self.font_cache
            .entry((id, weight, hash))
            .or_insert_with(|| {
                #[cfg(feature = "std")]
                unsafe {
                ...
                if let Some(font) =
                    Font::new_with_variations(&self.db, id, weight, &variations.coords())
                {
                    Some(Arc::new(font))
                } else {
                    log::warn!(
    ...
    pub fn get_font_by_variation_hash(
        &mut self,
        id: fontdb::ID,
        weight: fontdb::Weight,
        variation_hash: u64,
    ) -> Option<Arc<Font>> {
        if let Some(font) = self.font_cache.get(&(id, weight, variation_hash)).cloned() {
            return font;
        }
        // A miss for a non-zero hash must not fall back to the default
        // (wght-from-weight) instance: that silently remaps custom axes.
        if variation_hash == 0 {
            return self.get_font(id, weight);
        }
        None
    }
