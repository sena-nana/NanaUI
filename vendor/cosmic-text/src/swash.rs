...
fn apply_variations<'a>(
    scaler: swash::scale::ScalerBuilder<'a>,
    font: &crate::Font,
) -> swash::scale::ScalerBuilder<'a> {
    let coords = font.variation_coords();
    if coords.is_empty() {
        return scaler;
    }
    let swash_font = font.as_swash();
    let settings: Vec<(swash::Tag, f32)> = coords
        .iter()
        .map(|(tag, value)| (swash::Tag::from_be_bytes(tag.to_be_bytes()), *value))
        .collect();
    scaler.normalized_coords(swash_font.variations().normalized_coords(settings))
}
...
fn swash_image(
    ...
    let Some(font) = font_system.get_font_by_variation_hash(
        cache_key.font_id,
        cache_key.font_weight,
        cache_key.font_variation_hash,
    ) else {
    ...
    let scaler = apply_variations(scaler, &font);
