...
impl FontVariations {
    ...
    /// Stable identity for font/glyph caches. `0` means "no extra axes".
    pub fn hash64(&self) -> u64 {
        ...
        for variation in &self.variations {
            hash ^= u64::from(variation.tag.to_u32());
            hash = hash.wrapping_mul(0x1000_0000_01b3);
            hash ^= u64::from(variation.value.0.to_bits());
            hash = hash.wrapping_mul(0x1000_0000_01b3);
        }
        hash
    }

    pub fn coords(&self) -> Vec<(u32, f32)> {
        self.variations
            .iter()
            .map(|variation| (variation.tag.to_u32(), variation.value.0))
            .collect()
    }
}
