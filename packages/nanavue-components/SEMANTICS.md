...
| `<video>` / `nana-video` + `data-nana-video` | Video | Runtime `Video`（`nana.video`）；宿主经 `VideoRuntime` 推帧（`video:{id}` host-texture 槽）。有槽时不画 `poster`；无槽才走 `content_image` poster。`autoplay`/`loop`/`muted` 为声明式属性，`play`/`pause`/`ended` 由宿主派发 |
