...
`run_runtime` 会为主窗口和每个工具窗口分别应用、刷新与清理材质。窗口若 `WindowSettings.transparent` 为 true，则申请 `Transparent`（透明表面与 `[0,0,0,0]` 清屏），不以主窗口 Appearance 的 Solid 覆盖。macOS Scene GPU 申请
