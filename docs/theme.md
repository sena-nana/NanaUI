...
| 补做 | 原来的样子 |
| --- | --- |
| **L3 re-export 面** | `ThemeDefinition` / `CompiledTheme` 导出了，它们**由之构成的 token 结构体一个都没有**。消费方能拿着一份定义调 `.compile()`，却没法构造或修改它：每个 `with_*` builder 收的类型都叫不出名字，`ThemeId` / `ThemeSchemaVersion` / `ThemeGeneration` 三个 identity 字段全够不到。`nana_ui::theme` 现在镜像 `nana_ui_core::lib` 的那一份清单，外加 `SemanticColorMix` / `PaintStyle` / `BoxShadowSpec`，以及 `PaintStyle::mask` 要用的 `MaskImage` / `CssGradient` / `LinearGradient` / `RadialGradient` / `GradientStop`。 |
...
| **`NodeStyle::corner_radii`** | 逐角圆角只有 `paint.border_radii`，收的是长度。两块拼成一体的形状（外侧圆角、接缝直角）只能在构造期把 `radius_*` 花成数字，结果「圆角」设置一改，同一个窗口里所有单值圆角都跟着动，这些拼块留在原地。`corner_radii: Option<[Option<RadiusTier>; 4]>` 是 `radius` 的四角形式（`None` 是直角：接缝处哪怕用最小一档，两段弧分开的地方也会留缺口），同一条 intent 解析路径、同一次装主题时重解析；设了就压过 `radius`，和画的时候 `border_radii` 压过 `border_radius` 一致。 |
...
| **`SettingsRow::divided` 是分隔线,不是框** | `divided` 设的是统一的 `border_width`,四条边都描:行一行接一行排下来时上下边叠在一起,看着像分隔线,但一张卡里只有一行时就是圆角卡片里的一个直角方框。现在只描底边,一组的最后一行不描。修的时候还发现画笔的逐边宽度是在**不带**已解析颜色的情况下读的:颜色来自 `SemanticColorRole` 时 layout 上没有颜色,四边都读成 0,quad 就退回按统一宽度描四边——所以任何用语义色的单边边框都会被画成框。逐边宽度现在和统一宽度取自同一个颜色。像素基线 13 张、语义基线 8 条有意更新,都是设置行所在的页面;画笔修复没有改动其它任何快照。 |
