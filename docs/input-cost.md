...
> **后续：这条已经量完并结掉了两笔账，见
> [`runtime-dirty-frame.md`](runtime-dirty-frame.md)。** 把它扫成"脏节点数 × 总节点数"的
> 二维网格之后，结论比上面这段更细：纯绘制的改动本来就是 O(改动量)；只有**牵动布局**的
> 改动才是 O(总节点数)，原因是无障碍投影按被调度的布局脏集播种（会一路展开到文档根），
> 以及布局引擎对每个兄弟节点重复解析 effective style。两处已修，牵动布局的帧在 2,000 行上
> 端到端回到这份文档的基准：2,000 行 `reactive` 每事件 8.87 → **4.78** ms（−46%），
> settle 2.66 → **0.73** ms（**降到四分之一**）；`reactive-components` 5.45 → **1.79** ms
> （−67%）。`bare` 与 `listeners` 不变。
> 布局引擎的兄弟扫描随后也处理掉了：现在凡是"真正欠的工作是常数"的情况代价就是常数
> （502→8,002 节点都在 0.008–0.011 ms），只有"下面每一行真的要移位"那种还是 O(N)。
>
> **另外，这份文档里"Vue 层已经按改动量收费、不要再去优化 Vue 层"这句话是错的，需要修正。**
> 加了阶段计时并跑 500 / 2,000 两个规模之后：一次 hover 事件的 settle 里 73% 在 Vue 层，
> 而且**有 4 处随文档线性增长**——`try_bind_registered_component`（4.2×）、
> `find_sidebar_reparent_host`（5.4×）、`sync_layout_containing_blocks`（7.3×）、
> 以及 Vue 灌给 Runtime 的脏工作本身（`flush_runtime_systems` 3.9×）。
> **投影的集合是增量的（恒为 5 个 widget），花的时间不是。**
> 细节见 [`runtime-dirty-frame.md`](runtime-dirty-frame.md)。
>
> 同时**上面把这块成本挂到那两条未决门禁上是错的**：那两项量的是全量/首次通道，本轮两处
> 修复只作用在增量通道，实测前后纹丝不动（1.975→1.992、1.763→1.847 ms）。
