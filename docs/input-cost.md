...
把 Vue 压成常数之后，剩下线性的那条是 Rust L3：每指针事件 0.0065（250 行）→ 0.049 ms
（2,000 行），约 24 ns/节点。2,000 行里只有 20 行可见，指针也不改变任何布局。

三个判别实验把它框住了（都用真计时，不是跳过某段）：

| 问题 | 做法 | 结果 |
| --- | --- | --- |
| 是 `flush` 吗？ | `hover_xy` 之后再空跑一次 `flush` | **不是**。空闲 flush 0.0001 ms，且不随规模变——Runtime 自己的帧门挡得很好 |
| 是 hover 转换吗？ | 同一行再悬停一次 | **不是**。2,000 行上 0.0473 vs 换行的 0.0476 |
| 是命中目标查找吗？ | 把指针移到树外面 (-500, -500) | **不是**。0.0470，一样贵 |

所以是**每个指针事件都跑、与是否命中和是否换目标都无关的一段全树工作**——和这一轮在 Vue
侧修掉的那两处是同一个形状。它不在 `rebuild_hit_test`：那个函数在产品输入路径上没有调用方
（`runtime_input.rs` 里的调用全在测试模块）。

**具体是哪一段还没定位。** 到此为止的结论都是黑盒判别得到的，`RuntimeInputAdapter::dispatch_with_shaper`
本身有一千多行，需要在里面分段计时才能指名道姓。

一个测量陷阱值得记下来：`AppContext::last_frame_profile()` 和 `last_work_counters()`
**保留最后一次非空闲的值**（`finish_frame_profile` 里 `if profile.any_stage_ran()`）。
在空闲帧上读它们会拿到挂载帧的数据——第一次量的时候正是这样，读出"每事件 TextShape
12.3 ms"，比实测的整个事件还大 250 倍。
