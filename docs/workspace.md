...
`Tabs::on_reorder` 用「被移动的值 + 其后的值（`None` 表示末尾）」描述结果。跨窗口拖 Tab 用 `TabDragGroup` / `TabDragSurface`；选择、顺序仍由应用持有，可用 `DockWorkspace::save_to_store` 写进 `ViewStateStore`。接收方可用 `accepts_external_drop(false)` 只允许拖出、拒绝外部落入。
