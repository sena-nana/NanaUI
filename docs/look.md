...
标准正文 13px（`UI_BASE_TEXT_SIZE` / `type_scale::BODY`）。产品字号阶梯是 `nana_ui::theme::type_scale`：`HINT` 11、`META` 12、`BODY` 13、`SECTION` 14、`HEADING` 16、`TITLE` 18、`DISPLAY` 20，字重 `REGULAR` 400 / `MEDIUM` 500 / `SEMIBOLD` 600 / `BOLD` 700。间距走 `nana_ui::theme::space`（2px 网格；1/3/5/7 不新增档，接到 `XXS`/`XS`/`SM`）。1px 描边走 `HAIRLINE`，不是 spacing 档。标题栏 36px，窗口按钮 28px，侧栏导航行 28px。多行内容按内容长，里面的单行操作仍走这三档。
