...
- `AppContext::has_focused_ime_composition` is public. While it holds, `RuntimeInputAdapter` consumes the focused editor's navigation keys (arrows, Home/End, PageUp/PageDown) for every editor, not only numeric ones, so a composing field inside a table, tree or select no longer loses focus to that navigation. `RuntimeInputAdapter` already does this.
...
- **NumberInput editing:** the draft is a full editor.
  ...
  - Accessibility `Click` focuses it, `SetSelection` selects in the draft, and `SetValue` parses, snaps and clamps the text by the field's policy and commits it (emitting `NumberChanged`). It reports success when the field took the value: it moved (clamped or snapped as needed) or already held exactly that number. A request the bounds refuse outright (150 on a field already at its maximum of 10) reports failure. Text that is not a number is refused. Before, all three returned `false`.
