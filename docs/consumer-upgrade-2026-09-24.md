...
- `NumberFieldSpec::snap` snaps to the nearest grid point the field can display within its bounds. A bound off the grid (a maximum of 10.3 on a whole-number grid) holds at the last point inside it (10), and a grid the precision cannot display exactly (a minimum of 0.05 at one decimal place) snaps to its points as displayed. Snapping is idempotent and what `format` shows `parse` reads back. Before, `snap(11)` returned 10.3 and `snap(10.3)` returned 10. `can_increment` / `can_decrement` report whether a step on the grid can still move.
