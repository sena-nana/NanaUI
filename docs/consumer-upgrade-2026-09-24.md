...
- `RuntimeInputAdapter` no longer types the control characters hosts report with command keys (`"\r"` for Enter, `"\u{1b}"` for Escape) into a focused editor when no editor claimed the key. Tab and newline text still types. Before, an Enter or Escape that a NumberInput did not use landed in its draft. A host with its own text fallback should drop them the same way.
