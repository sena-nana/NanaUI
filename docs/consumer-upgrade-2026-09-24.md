...
- `NumberFieldSpec::snap` clamps onto the grid's reachable range: a maximum off the grid (10.3 on a whole-number grid) holds at the last grid point inside it (10), so snapping is idempotent and a field never stores a number it cannot display. Before, `snap(11)` returned 10.3 and `snap(10.3)` returned 10. `can_increment` / `can_decrement` report whether a step on the grid can still move.
