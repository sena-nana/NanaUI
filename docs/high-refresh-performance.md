...
随后启用 HoverCard 生命周期模块并完成全量回归：**853 项通过，0 项失败**。新增覆盖
触发器延迟开启、内容移入保持开启、离开后的宽限关闭、Escape 关闭，以及头像纹理槽
与占位样式；固定定位内容会在开启后重新投影，避免子内容把触发器撑大。

活动运行时浮层的排序查询也完成了一次局部优化：每次查询先建立当前文档的
`StableNodeId → document order` 索引，宿主与活动根的顺序查找从每个浮层重复扫描改为
