...
### 挂载:每个节点跑 5 遍 CSS 级联,但合并掉只值 2.5%

追 Vue 挂载(2,000 行,192.7 ms)的时候把它整条拆开了:

```
192.7 ms
├─   8.4  引擎/宿主构造
├─  75.6  initialize(解析 + Vue 求值 + .mount())
│    ├─ 36.7  V8 侧(Vue 造 vnode + renderer 的 JS)
│    ├─  6.4  编解码 v8↔host
│    ├─  1.0  注册表 dispatch + trace
│    └─ 42.0  handler 内部
│         └─ 30.4  ← 10,001 次 CSS 级联,约每节点 5 遍
└─ 108.1  首帧(全量布局 + 场景构建)
```

`insert_child` 尾部无条件 `reapply_layout_for(child)`,`patchProp` 也各触发一遍,加上邻域与
svg 的重级联——2,001 个节点跑了 10,001 次,每次约 3 µs。

**做了延迟合并的实验**:`reapply_layout_for` 在批内只记入待处理集,由帧边界
(`snapshot` / `take_snapshot_changes` / `resolve_document_layout` /
`sync_layout_containing_blocks`)统一 drain。差分哈内斯这次**是有牙的**——它驱动**真正的
host op**(经 `register_dom_host_ops_with_bridge` 注册的那些闭包,不是底下的 bridge 方法,
因为好几个 handler 自己就读 `props.layout`、属于批内读者),比对的是**每个节点的 LayoutBox**
而不是级联后的样式。两处沙盘在 default 与 `scene-view` 两种 feature 下都会让它失败:
丢掉一个待处理节点、以及把全部 drain 拿掉。

**还是撤了,两个理由,第二个是决定性的。**

一、**契约。** 默认打开之后 90 个既有测试挂掉,其中 79 个是 `bridge::tests` 直接读
`props.layout`——它们编码了一条契约:*每次 bridge 调用之后 `props.layout` 就是最新的*。
而 `MessageBridge::get(&self)` 是 `&self` 直接交出 `props.layout`,**没有任何办法让延迟对它
透明**。这和包含块那条是同一个形状:缺一个写入/读取的唯一收口。

二、**钱不在那儿。** 打开延迟之后实测挂载 195.1 → 190.3 ms,**只省 4.8 ms(2.5%)**,不是我
估的 ~24 ms。我估错的地方是:我假定那 5 遍级联是同一个节点重复 5 次,按 id 去重就能 5→1。
实测说明大部分不是重复(邻域与父链级联的是**别的**节点),或者批内本来就有 drain 把合并打断
了——我没有把这一步追到底,因为 2.5% 已经不值得为它改契约。
...
**Vue 挂载每行 ~91 µs,对 L3 的 ~15 µs。** 上一节把它拆开了:跨界不是问题(编解码 6.4 +
dispatch 1.0,不到 4%),级联合并也不是(2.5%)。剩下的是两边各自的真实工作——每行约 21 µs 的
宿主活加约 18 µs 的 V8 活,以及首帧 108 ms 的全量布局与场景构建。要动就得动 bridge 里
`createElement` / `patchProp` 每次做了什么,那是和这一轮同类的活。
...
结果反而是对的。守卫区分不了这两种情况，所以分诊完就移除了。容器侧声明 `ALWAYS_REPROJECT`：
相等的写入也投影一次，把被冲掉的补丁补回来，投影不产生写入时照样跳过提交、生命周期和
assembler。视觉层面由像素快照（620/620）和语义快照（146/146）兜底。
