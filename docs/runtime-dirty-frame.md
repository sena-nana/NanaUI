...
- **内容驱动尺寸的容器**：第五轮补上了 `ContainerPlan` 的测量侧对应物 `MeasurePlan`。
  8,002 个节点上 1.41 → 0.011 ms（**128 倍**），与文档大小无关。但**端到端一开始是 0**——
  真实 Vue 树里三个写者对"默认的列"用了两种拼法，每帧把计划打死；改成比较布局等价而非
  拼法之后，settle −18%、Layout −51%。仍欠的是"孩子真的改了尺寸"那一半，见"还剩什么"。
...
2,000 行，`--shape window`，p50 ms/事件（第五轮那列见"第五轮"一节，同机三轮重测）：

| 变体 | 起点 | 第一轮 | 第三轮 | 第四轮 | 第五轮 | 累计 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| `reactive` 每事件 | 8.872 | 6.660 | 6.038 | 4.783 | **≈4.96** | **−44%** |
| 　其中 settle | 2.664 | 1.638 | 1.330 | 0.732 | **0.630** | **−76%** |
| `reactive-components` 每事件 | 5.449 | 3.388 | 2.751 | 1.793 | **≈1.77** | **−68%** |
| 　其中 settle | 2.485 | 1.537 | 1.248 | 0.623 | **0.488** | **−80%** |

第五轮只动 settle：`reactive` 的每事件被 V8 dispatch 主导，settle 降 0.14 ms 在里面看不出来
（4.78 与 4.96 之间的差别是不同机器状态下的重测，不是回归；同机同轮对照是
0.767 → 0.630）。
...
**这条只治了缓存这一侧。** 三个写者每帧互相覆盖 2,003 个节点这件事本身还在,
`project_migrating_component` 0.163 ms + `try_bind_registered_component` 0.161 ms
仍然占 settle 的四成——那是另一笔账,见"还剩什么"。

## 还剩什么

**三个写者对同一份布局的写入战。** 上一节定位到但没有修:`register` 播种、cascade 重发、
`Stack` 投影回写,每个指针事件在 2,003 个节点上来回覆盖。修它要决定"谁赢",是 bridge 的
语义决策,有自己的门禁(559 张快照、组件资格),不该顺手做。它现在的直接代价是
`project_migrating_component` + `try_bind_registered_component` ≈ 0.32 ms/事件,
settle 0.63 ms 里的一半。
