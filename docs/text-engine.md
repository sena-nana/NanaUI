...
- **截断的单行向布局报整行宽度**。Runtime 在盒子已测量后按旧盒宽塑形，画出来的是裁过的行；
  但 `TextMetrics::width`（布局读到的内在宽度）取不带宽度约束再排一次的整行宽度。否则
  shrink-to-fit 的父级会一直拿着旧盒子：标签改成更长的文字只会被继续裁短，永远长不回来。
  这是「度量与保留的 layout 同一来源」的唯一例外，只在置了 `ELLIPSIZED` 的单行上发生。
...
Runtime 在边界上换算一次，全部由 `EditorFrame` 负责——它不分横竖，每个编辑器都有一个：内容盒（扣掉
数字输入框的步进按钮）、行空间里的沿行 / 跨行滚动、单行输入框的跨行居中与行首锚定（横排 LTR 贴左、
RTL 贴右、竖排贴顶、竖排 rtl 贴底，都是一个滚动量）。单行输入框溢出时只滚到露出光标为止，其余时候行首保持可见：LTR 从左端显示，RTL 从右端、竖排 rtl 从底端显示——同一条规则的镜像，不记录光标以外的状态。组件几何的每个矩形与指针命中都走它，横排不再
另算一套滚动。单行输入框的编辑几何没有行预算，文字区域就取整行长度，画笔的 `start` 对齐没有余量，
挪不动字形；多行编辑器的文字区域取内容盒在行方向上的长度，与编辑几何的折行预算相同。
...
- **横向滚动交给画笔当平移**：多行编辑器的几何另带 `scroll`（`EditorFrame::scroll_x`），见「编辑器的横向滚动也是平移」。
...
- **方向键**：意图（`TextCaretIntent`）约定在行空间里：左右沿行、上下跨行。宿主报的是物理键，
  ...
  修饰键就整套跟着走：Cmd+↑/↓ 是沿列的行首行尾（与横排的 Cmd+←/→ 一样是逻辑行），`vertical-rl`
  里 Cmd+← 是文档末尾、Cmd+→ 是文档开头，Alt/Ctrl+↑/↓ 按词移动，Alt+←/→ 移动行块、
  Alt+Cmd+←/→ 在相邻列加光标。PageUp/PageDown 的一页是跨列方向的视口宽度
  （`text_input_page_extent`）。
- **竖排 rtl**：行首在底端。多行编辑器的编辑几何与画笔共用内容高度这一行预算，各自把每列的
  `start` 对齐到底；单行输入框的编辑几何没有行预算，文字区域就取整行的长度（画笔的 `start` 对齐
  没有余量，挪不动字形），再由坐标框用负的沿列滚动把它贴到底端。方向键沿用横排 RTL 的约定：段落
  自下而上读，↑ 是往前，走过一列顶端接着进入下一列的底端，↓ 反之；列内仍按视觉逐格走。段落
  末尾位置的光标落在 line-left（顶端），与横排 RTL 把末尾画在左边同理。
- **滚动**：记录的 `ScrollOffset` 与所有滚动容器一样是物理偏移，`ScrollMetrics` 由塑形结果
  （`text_scroll_metrics`）实时给出：`vertical-rl` 的列从右边排起，后面的列向左溢出，所以跨列偏移
  从 0 往负走（CSSOM 的负 `scrollLeft`），滚轮、`scroll_to` 与夹取和其他容器走同一条路。记录的
  偏移是**请求**（组件字段投影、滚轮、minimap 写入）；画笔与命中在 `editor_frame` 里把它夹进值的
  范围、换成行空间的沿列 / 跨列两个量，聚焦的多行编辑器再按横排同样的规则把光标滚进视口。
  `UiWorld::scroll_offset` 报的是这个画出来的位置，所以滚轮、脚本读到的就是屏幕上的位置：打字后
  滚轮从光标所在处接着滚，值变短后第一格就动。视口钉住比的是请求（`scroll_request`）。
...
### arena 与画序索引表：画序与存储序不是一回事（#224）

instance 放在 storage buffer 里，draw 不直接走它。draw 走的是一张**按画序排列的
`u32` 索引表**（per-instance 顶点缓冲），顶点阶段读到一个槽号，再从 storage buffer
取那个槽的 instance。两张表各由一个 `SlotArena` 发位置，块一直归它的 entry 所有：

- **arena**：instance 在 storage buffer 里的位置。一段文本变了，动的只有它自己那几百
  字节；**长出了块也一样**——它换到 arena 别处，写的仍然只是它自己的块。还回来的空间
  与左右相邻的空间合并、按最合适的大小切给下一块，碰到末尾就直接缩短 arena，所以长大
  的段落留下的小洞会被后来的块用掉，而不是把 arena 推向整理。arena 只在「放不下」或
  「空洞过半」时整理，与画序无关；整理后留三分之一空位。「放不下」按本帧要放的块在现有
  空洞里逐个试放（与真正分配同一个最合适规则，不拷贝空洞表）来判断，宁可多整理一次，
  不能让缓冲在帧中途被换掉。
- **order**：entry 在索引表里的区间。区间逐槽指向它的块（余量那几槽指向尺寸为 0 的
  instance，画不出像素）。相邻的两个区间合成一个 draw，不相邻就多一个 draw；
  攒够预算（每 1024 个索引槽一个 draw），或者空洞占了一半，就按画序把**索引表**整理
  一次——四字节一槽，一个 instance 都不动。
- 块按 size class 留 1/8 的余量（至少 4 个槽），余量填成尺寸为 0 的 instance。
  打字打到第十二个字符不会越级。
- **区间与块一样大，不给每段留增长余量。** 余量能让一段文字在画序里原地长大、不拆
  draw，但每个空着的索引槽仍是顶点阶段要跑的一个四角 quad（本机约 2 ns 一槽）。量过：
  每个区间留两倍余量，draw 从 90 降到 1、索引字节从 179 KB 降到 21 KB，GPU 时间却多
  约 20%。break 预算也量过：每 256 槽一个 draw 能让索引字节减半，draw 翻三倍、GPU 慢
  约 2%，不值。
- **文字长度在变的地方，整理时留成片的空隙。** 整理画序时看这段时间里是什么让区间换了
  位置：过半是段落长出了区间（表格、日志），新布局每 512 槽留 128 槽空隙，空隙填
  `VACANT_INDEX`，顶点阶段不读 instance、直接剔除，draw 可以直接跨过它。之后一段文字
  长大，就从紧跟在后面的空隙里取；隔着几段，就把中间那几段往后挪进空隙（只重写它们的
  索引）——原地长大，不拆 draw。空隙约占两成索引槽，远少于每段留两倍余量的一倍半。
  是段落来来去去（滚动、转进转出视口）让区间换位置时不留：空隙挡不住它们拆 draw，
  只会让每次整理更大。区间缩回时保留原区间，多出的槽写 `VACANT_INDEX`。
- **文字停止变长后，空隙还回来（#230）。** 空隙本来只在下一次预算触发的整理时才可能消失，
  表格刷完一轮就不再整理，之后每一帧都在为用不上的空隙跑 quad。现在一个 target 连续
  `SETTLE_FRAMES`（60）个要画的帧没有区间长大，就做一次不留空隙的整理，只重写索引表，区间
  缩回留下的余量一并收回。它与预算整理是同一个判断，一帧最多整理一次；安静期里的任何整理都
  不留空隙。文字恢复变长后，要等 break 攒够预算，下一次整理才重新留空隙，不会每帧来回。
  - 按帧、不按时间计，因为空隙的成本按画的帧累计。一万标签停止变化后 quad 152 085 → 136 045
    个/帧，draw 15 → 1，GPU 2.86 → 2.72 ms（负载 < 4、交替 10 轮取 min，
    [数据](performance-data/text-gap-reclaim-2026-09-24/README.md)）；那次整理重写 544 KB 索引，
    帧耗时里看不出尖峰，几帧空隙就抵得上它。60 帧是它的数倍，间歇刷新的文字留得住空隙。
  - 复用上一帧批次的帧（场景没变、旁边有 HostTexture / GPU 视图）照样画文字、付空隙，却不经过
    flush，所以也计入安静帧；到期那一帧不复用批次，重建一次。批次里没有文字时不计也不重建。
  - 设备丢失与字体注册让画序整体作废时，一并清掉「留空隙」标记与长大计数。
- 整理会推进各自的代际；索引只认槽号，所以 arena 整理后没搬的块连索引都不用重写。

画序不变、块不动的帧，索引表上传是 0 字节。

实测（`docs/performance-data/text-draw-order-2026-09-22/`，一万标签、每帧随机 1% 的行换
随机长度文本、60 帧）：instance 上传 1 136 190 → **40 300** B/帧，也就是被重建段落自己的块
（每段约 410 B）；一千标签那一档是 9 363 B/帧——标签多十倍、字节多 4.3 倍，不随屏上字形
总数增长；新增的索引上传 65 107 B/帧；
合计约为前的九分之一，draw 87 → 21。跑 600 帧时是 50 KB 与 25 KB、36 个 draw。整片旋转的
`transform` 原来每帧半兆字节的 instance 整理变成 87 KB 的索引整理，instance 为 0，draw 不变。
其余格子两项都是 0。CPU 指令数：有重建的格子少 2–5%，每帧只写一小块的格子多约 1%（见
「in-flight」）；GPU 时间交替 A/B 在噪声内。画廊 620 帧逐字节不变。

代价与边界：

- 顶点阶段多一次依赖读取（槽号 → instance）。槽号走顶点缓冲的定长取数，只有
  instance 是一次 storage 读。
- 顶点阶段的 storage buffer 从两张变三张（run 表、presentation 表、instance），
...
但**每次调用都有固定成本**：Metal 上约五万条指令，远多于它搬的字节——一帧重建一百个
标签就是一百次。所以 instance 块和索引区间不再各自 `write_buffer`：一帧要写的全部排进
每个 target 自己的一段 staging ring（一到两次 `write_buffer`），再在宿主给的 encoder 里
逐块 `copy_buffer_to_buffer` 到位，排在读它们的 pass 之前。`mutate-1pct` 一万标签的指令数
因此少 5%。
...
text_index_upload_bytes       // 画序索引表（#224）：区间新放置或块搬了才有
text_index_slots_drawn        // draw 跨过的索引槽，即顶点阶段要跑的 quad，空隙与余量槽都算（#230）
...
- `capacity_for` 留的 1/8 余量过去从来没用上：`begin_build` 要求容量**精确相等**才留
  ...
  它自己的字节：它落到 arena 别处，draw 被拆开，拆够了就整 arena 重排。（#224 之后
  重排的只是画序索引表，见「arena 与画序索引表」。）
...
**编辑器的横向滚动也是平移。** 多行编辑器的滚动是它自己的，Runtime 过去把它直接算进值、
光标、选区的页面坐标，画笔分不出滚动，长行横向小数滚动时整段每帧重解析。现在几何另带
`scroll: Option<TextInputScroll>`（只有多行编辑器有）：`offset_x` 是滚动在页面 x 上的位移，
`text_x` 是值不含滚动时的精确 x（`text.bounds.x - offset_x` 会差一个 ulp，相位就变了）。
页面坐标的几何不变，命中、IME、无障碍照旧读它。scene 提取（`EditorScroll`）把随文字沿 x
移动的图元画成「盒子减去 `offset_x`、再乘一层平移」，画笔照常吸附，值、光标、选区与其上的
标记挪同样多的整像素；gutter、当前行底条、minimap、sticky 行、弹层不动。只吸附文字不够：
光标 `round(c − s)` 与字形 `c − round(s)` 会在部分帧差一像素。

竖直方向仍在盒子里（行框顶边本来对齐整像素）；单行输入框没有 `scroll`（滚动只跟随光标）。
逻辑偏移保留小数。验证：`a_fractional_horizontal_scroll_of_an_editor_keeps_every_glyph`
（1×/1.1×/1.2×/1.75×，每帧 −0.37 px，内容区逐像素整体平移，零重建）与
`a_vertical_rl_editor_hands_its_column_scroll_to_the_painter`。剩下的：诊断行尾文案和 atom chip
标签的未滚动 x 是还原的，可能在部分帧重解析，数量是屏上的条数，不是整段。
...
| ~~编辑器横向滚动仍重解析~~ | 已做：沿 x 的滚动由 `ComponentGeometry::TextInput::scroll` 以平移交给画笔，见「编辑器的横向滚动也是平移」 |
