...
| Workspace Region | 已完成本阶段 | 动态注册/注销、自定义 ID、role/placement/scope、可见性、折叠、尺寸约束、响应式策略、拖动调整、双击复位、JSON 持久化 |
| Region 几何 | 已完成本阶段 | 任意注册区域的逻辑/物理像素矩形与窗口 DPI；折叠释放空间，overlay 不占用 primary |
| 工作区 Demo | 已完成本阶段 | Code/Github/Live2D 三套真实区域注册与交互状态，Demo 不再拥有布局编排逻辑 |
| LiliaUI 风格主题与壳层 | 已完成本阶段 | dark/light 语义令牌、36px 标题栏、紧凑导航和工作区表面层级 |
...
```text
NanaUI  cargo test --workspace --all-targets --locked       44 passed / 9 suites
...
`ui-snapshots --locked` 已在 WGPU 30 上成功生成全部十一张验收图，新增 loading
组合按钮和交互式 Card，用于验证按钮内容双轴居中与卡片边缘对齐。
...
此外，当前本地三仓修改尚未提交。只有在 Iced、Cryoglyph 的 WGPU 30 修订可从
远端获取，并将 NanaUI 清单固定到这些 revision 后，三平台 CI 才能从“配置存在”
升级为“可复现并已运行”。
