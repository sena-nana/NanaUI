...
| 可复用工作区框架 | 已完成本阶段 | `WorkspaceController`、`Workspace`、`DesktopShell` 与 `app_shell` |
...
`ui-snapshots --locked` 已在 WGPU 30 上成功生成 23 张验收图，覆盖 1280×720
单侧栏 Workspace、Sidebar、Settings、可编辑圆角及其运行时应用、主区域
start/end 贴边圆角及禁用状态、loading 组合按钮和交互式 Card。
