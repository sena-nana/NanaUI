...
**层叠。** 同属 author。样式表内部先 normal 再 `!important`（再比特异度和源序）。prop / inline **普通**声明覆盖样式表普通声明（inline 覆盖 prop）。样式表 `!important` 覆盖 prop / inline 普通声明。prop / inline 上的 `!important` 会去掉标志并作为 author-important 再写：覆盖样式表 `!important`，且 inline important 覆盖 prop important。`:hover` / `:focus` / `:active` 进交互桶，按 Runtime 指针 / 焦点状态叠到绘制样式上，不当布局条件。`::before` / `::after` 在有 `content` 时生成匿名子盒（部分落地：字符串 content，无 `attr()` / 计数器 / `quotes`）。`@keyframes` 已解析，并由交互层做成绘制动画（opacity / color / transform / filter 等 paint 字段）。
