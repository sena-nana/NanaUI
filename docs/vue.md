...
受管的不只是 JS：同一个 `fetch_host` 也是引擎自己那条资源出口——`url()` 图片、`<img src>`、`mask-image`、`border-image` 走的是同一份 `FetchPolicy`，同样逐跳复核重定向。没注入 host 就一张远程图都不取，`data:`、`file:` 与相对路径不受影响。`@font-face` 不取远程，只认 `local()`、`data:` 和 jail 内的本机文件。
