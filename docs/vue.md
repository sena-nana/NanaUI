...
一个 render function 拥有整列时，hover 处理器改一个参与渲染的 `ref` 会让 Vue 重建整列的 vnode——实测 2,000 行时一次鼠标移动 9 ms（超过半帧），是同一棵树无处理器时的 140 倍。模板里的 `v-for` 编译出来就是这个形状，不生成子组件。这是长列表上最容易写出来的写法，也是这条路径上目前唯一还随树增长的成本。把行拆成各自的组件，或者让 hover 只改不参与渲染的状态（后者是常数成本）。处理器提到渲染之外缓存住身份还能再省一层。
