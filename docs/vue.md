...
一个 render function 拥有整列时，hover 处理器改一个参与渲染的 `ref` 会让 Vue patch 整列——实测 2,000 行时一次鼠标移动 18.7 ms（超过一整帧），是同一棵树无处理器时的 48 倍。这是长列表上最容易写出来的写法，也是这条路径上目前最大的单项成本：每个事件 4,001 次 `patchProp`，因为内联 `style` 对象和内联箭头处理器每次渲染都是新的引用。把行拆成各自的组件，或者让 hover 只改不参与渲染的状态。渲染器侧也还有一条没做的修法（按值而非按引用比较 style），见 [输入成本](input-cost.md)。
