...
fn ctx<'a>(
    ...
    let (sibling_index, sibling_count) = sibling;
    let (of_type_index, of_type_count) = of_type;
    MatchContext {
        tag,
        id,
        classes,
...
fn cascade_once() {
    ...
    for i in 0..TREE_CARDS {
        ...
        apply_stylesheet_to_layout(
            ...
            rules,
            &ctx(
                node("div", &shell, &empty_attrs, false, false),
                &[],
                &[],
                (0, 1),
                (0, 1),
        ...
        apply_stylesheet_to_layout(
            ...
            rules,
            &ctx(
                node("input", &none, &empty_attrs, true, i % 2 == 0),
        ...
        apply_stylesheet_to_layout(
            ...
            rules,
            &ctx(
                node("span", &none, &empty_attrs, true, false),
                std::slice::from_ref(&parent),
                &[],
                (1, 4),
                (0, 2),
        ...
        apply_stylesheet_to_layout(
            ...
            rules,
            &ctx(
                node("p", &card, &empty_attrs, false, false),
                std::slice::from_ref(&parent),
