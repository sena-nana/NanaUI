...
impl NodeSlots {
    ...
    /// The operation offsets these slots occupy, appended to `out`.
    fn extend_offsets_into(&self, out: &mut Vec<usize>) {
        match self {
            Self::One(offset, _) => out.push(*offset),
            Self::Many(slots) => out.extend(slots.iter().map(|(offset, _)| *offset)),
        }
    }
}
...
impl VisibilityIndex {
    ...
    pub(super) fn update(&mut self, scene: &UiScene, changed: &[StableNodeId]) {
        // Collected first, and into one `Vec` for the whole update rather than
        // one per node: the descent below needs `&mut self`, and the offsets
        // have to be sorted before they can be read as runs.
        let mut offsets = Vec::new();
        for node in changed {
            if let Some(found) = self.nodes.get(node) {
                found.extend_offsets_into(&mut offsets);
            }
        }
        if offsets.is_empty() {
            return;
