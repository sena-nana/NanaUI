//! Workspace **region** layout contract (Style Model Layout — shell regions).
//!
//! Distinct from box flex layout ([`crate::LayoutStyle`] in `box_layout`).
//! Both are Layout-slice inputs to Nana drawing; neither is a CSSOM.
//! CSS parse for `LayoutStyle` stays in `nana-ui-vue::css_map`.
...
impl WorkspaceLayout {
    ...
    /// Builds a temporary layout snapshot with animated extents applied.
    ///
    /// Intended for renderer-side geometry sampling during collapse transitions.
    pub fn with_transient_extents(
        &self,
